...
import { NotSupportException } from './exception/not-support-exception';
import { WebGLProgramLinkException } from './exception/web-gl-program-link-exception';
import { WebGLShaderCompileException } from './exception/web-gl-shader-compile-exception';
import { SHADER_FRAGMENT_LOADING, SHADER_FRAGMENT_YCRCB_TO_RGBA, SHADER_VERTEX_IDENTITY } from './shaders';
...
export class GLRenderer implements IRenderer {
  ...
  public destory(): void {
    ...
    // remove listener
    this._canvas?.removeEventListener(`webglcontextlost`, this.handleContextLostBound, false);
    this._canvas?.removeEventListener(`webglcontextrestored`, this.handleContextRestoredBound, false);
  ...
  private createContext(): WebGLRenderingContext {
    ...
    if (!gl) throw new NotSupportException('can not create webgl rendering context, please check your browser!');
    return gl;
  }
  ...
  private registerEvent(): void {
    this._canvas?.addEventListener(`webglcontextlost`, this.handleContextLostBound, false);
    this._canvas?.addEventListener(`webglcontextrestored`, this.handleContextRestoredBound, false);
  }
  ...
  private initGL(): void {
    ...
    this._program = this.createProgram(SHADER_VERTEX_IDENTITY, SHADER_FRAGMENT_YCRCB_TO_RGBA);
    ...
    this._loadingProgram = this.createProgram(SHADER_VERTEX_IDENTITY, SHADER_FRAGMENT_LOADING);
