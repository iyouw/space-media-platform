...
export class Mp2Decoder extends Decoder {
  ...
  private decodeFrame(reader: BitReader, packet: Packet): boolean {
    ...
    for (let sb = bound; sb < sblimit; sb++) {
      this._allocation[0][sb] = this._allocation[1][sb] = this.readAllocation(sb, tab3, reader);
    }
  ...
  private readSamples(ch: number, sb: number, part: number, reader: BitReader): void {
  ...
  private matrixTransform(s: Array<Array<number>>, ss: number, d: Float32Array, dp: number): void {
    let t01,
      t02,
      t03,
      t04,
