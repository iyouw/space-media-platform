...
import { VideoFrame } from '../frame/video-frame';
import {
  CODE_BLOCK_PATTERN,
  DCT_COEFF,
...
export class Mpeg1Decoder extends Decoder {
  ...
  private _codedWidth = 0;
  private _codedHeight = 0;
  private _codedSize = 0;
  ...
  private _hasSequenceHeader = false;
  ...
  private _fullPelForward = 0;
  private _forwardFCode = 0;
  private _forwardRSize = 0;
  private _forwardF = 0;
  ...
  // Slice Layer
  private _quantizerScale = 0;
  private _sliceBegin = false;

  // Macroblock Layer
  private _macroblockAddress = 0;
  private _mbRow = 0;
  private _mbCol = 0;

  private _macroblockType = 0;
  private _macroblockIntra = 0;
  private _macroblockMotFw = 0;

  private _motionFwH = 0;
  private _motionFwV = 0;
  private _motionFwHPrev = 0;
  private _motionFwVPrev = 0;

  // Block Layer
  private _dcPredictorY = 0;
  ...
  private decodePicture(reader: BitReader, frame: Frame): boolean {
    ...
    if (this._pictureType <= 0 || this._pictureType >= PICTURE_TYPE.B) return false;
    ...
    frame.add(this._currentY!);
    frame.add(this._currentCr!);
    frame.add(this._currentCb!);
    this.onFrameParsed?.(frame);

    // if this is a reference picture then rotate the prediction pointers
    if (this._pictureType === PICTURE_TYPE.INTRA || this._pictureType === PICTURE_TYPE.PREDICTIVE) {
  ...
  private decodeMacroblock(reader: BitReader): void {
    ...
    if (this._sliceBegin) {
      ...
      while (increment > 1) {
        ...
        // assign yuv
        this.copyMacroblock(this._motionFwH, this._motionFwV, this._forwardY!, this._forwardCr!, this._forwardCb!);
        increment--;
      }
      this._macroblockAddress++;
    }
  ...
  private decodeBlock(block: number, reader: BitReader) {
    ...
    if (this._macroblockIntra) {
      ...
      if (dctSize > 0) {
        ...
        if ((differential & (1 << (dctSize - 1))) !== 0) {
          ...
          this._blockData[0] = predictor + ((-1 << dctSize) | (differential + 1));
        }
      } else {
    ...
    if (this._macroblockIntra) {
      ...
      if (n === 1) {
        this.copyValueToDestination((this._blockData[0] + 128) >> 8, destArray, destIndex, scan);
        this._blockData[0] = 0;
      } else {
        this.idct(this._blockData);
        this.copyBlockToDestination(this._blockData, destArray, destIndex, scan);
        this._blockData.fill(0);
      }
    } else {
      // add data to the predicted macroblock
      if (n === 1) {
        this.addValueToDestination((this._blockData[0] + 128) >> 8, destArray, destIndex, scan);
        this._blockData[0] = 0;
      } else {
        this.idct(this._blockData);
  ...
  private readHuffman(codeTable: Uint8Array | Int8Array | Int16Array | Int32Array, reader: BitReader): number {
    let state = 0;
    do {
  ...
  private copyBlockToDestination(block: Int32Array, dest: Uint8ClampedArray, index: number, scan: number): void {
    for (let n = 0; n < 64; n += 8, index += scan + 8) {
  ...
  private addValueToDestination(value: number, dest: Uint8ClampedArray, index: number, scan: number): void {
    for (let n = 0; n < 64; n += 8, index += scan + 8) {
      dest[index + 0] += value;
      dest[index + 1] += value;
      dest[index + 2] += value;
  ...
  private addBlockToDestination(block: Int32Array, dest: Uint8ClampedArray, index: number, scan: number): void {
    for (let n = 0; n < 64; n += 8, index += scan + 8) {
      dest[index + 0] += block[n + 0];
      dest[index + 1] += block[n + 1];
      dest[index + 2] += block[n + 2];
